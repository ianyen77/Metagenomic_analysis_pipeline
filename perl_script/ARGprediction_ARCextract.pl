...
for ($x=0; $x<@ARGV; $x++){
	...
	system("diamond blastx -d ~/DB/Diamond/DB/SARG2.2_DB.dmnd -q $orf --id 80 -p 16 -e 1e-10 -f 6 -k 1 --query-cover 70 -o $diamondout");
	...
	system("diamond blastx -d ~/DB/Diamond/DB/SARG2.2_DB.dmnd -q $prodigalout_nucl  --id 80 -p 16 -e 1e-10 -f 6 -k 1 --query-cover 70 -o $ARCorfrediamondout");
