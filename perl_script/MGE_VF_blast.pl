...
for ($x=0; $x<@ARGV; $x++){
	...
	system("diamond blastx -d /media/sf_sf/DB/Diamond/v2.1.8_diamond_DB/MGEs_database.dmnd -q $orf -p 16 --id 70 -p 16 -e 1e-10 -f 6 -k 1 --query-cover 70 -o $MGE_diamond");
	system("diamond blastx -d /media/sf_sf/DB/Diamond/v2.1.8_diamond_DB/VFDB_coredataset.dmnd -q $orf -p 16 --id 70 -p 16 -e 1e-10 -f 6 -k 1 --query-cover 70 -o $VF_diamond");
	system("diamond blastx -d /media/sf_sf/DB/Diamond/v2.1.8_diamond_DB/bacmet_experiment.dmnd -q $orf -p 16 --id 70 -p 16 -e 1e-10 -f 6 -k 1 --query-cover 70 -o $bac_diamond");
	}
